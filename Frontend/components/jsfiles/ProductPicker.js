// src/components/jsfiles/ProductPicker.js
...
export function ProductPicker({visible, onClose, onAddProduct, products = []}) {
    ...
    const handleDone = () => {
        Object.values(selectedProducts).forEach(prod => {
            ...
            onAddProduct && onAddProduct(safeProd, safeProd.qty);
        });
        onClose && onClose();
    };
                ...
                onPress={() => {
                    setSelectedProducts(prev => {
                        ...
                        if (newSelected[String(item.id ?? item._id)]) {
                            delete newSelected[String(item.id ?? item._id)];
                        } else {
