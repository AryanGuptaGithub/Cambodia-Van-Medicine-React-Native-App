...
export default function SalesReturnTotalScreen({navigation}) {
    ...
    const salesWithNames = useMemo(() => {
        return sales.map((sale) => {
            const customer = customersList.find(
                (c) => String(c._id) === String(sale.customerId)
            );
            return {
                ...sale,
    ...
    const totals = useMemo(() => {
        ...
        filteredSales.forEach((sale) => {
            ...
            const returnAmount = sale.returnsAmount || 0;
    ...
    const renderItem = ({item}) => (
        <View
            ...
            <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
                <Text style={{fontWeight: '700', fontSize: 16}}>{item.invoiceNumber || item.invoice}</Text>
            ...
            <View style={{marginTop: 10}}>
                ...
                <Text style={{fontSize: 14, color: '#b91c1c', marginTop: 4}}>
                    Returns:{' '}
                    <Text style={{fontWeight: '700'}}>
                        ${(item.returnsAmount || 0).toFixed(2)}
                    </Text>
                </Text>
                ...
                <Text
                    ...
                    Net: ${((item.totalAmount || 0) - (item.returnsAmount || 0)).toFixed(2)}
                </Text>
            </View>
        </View>
    );
            ...
            <View style={{flex: 1, padding: 14}}>
                ...
                <View
                    ...
                    <Text style={{fontSize: 14}}>
                        ...
                        style={{fontWeight: '700'}}>${totals.totalSalesAmount.toFixed(2)}</Text> ({totals.totalSalesCount})
                    </Text>

                    <Text style={{fontSize: 14, marginTop: 4, color: '#dc2626'}}>
                        Total Returns Amount: <Text
                        style={{fontWeight: '700'}}>${totals.totalReturnsAmount.toFixed(2)}</Text> ({totals.totalReturnsCount})
                    </Text>
                ...
                <FlatList
                    data={filteredSales}
                    keyExtractor={(item) => String(item._id || item.id || Date.now())}
